...
const OffersReceived = ({ authenticated }) => {
  ...
  useEffect(() => {
    ...
    const getOffers = async () => {
      const result = await axios.get(`/api/offers/seller/${user.userId}`);
  ...
  const acceptOffer = async (offer) => {
    const result = await axios.put(`/api/offers/${offer.id}`, {
      request: 'accept',
      closed: 'accepted',
      seller_listing: offer.sellerlistings_id
    })
    console.log(result)
    setOffers(offers.filter(x => x.sellerlistings_id !== offer.sellerlistings_id))
  }

  const declineOffer = async (offer) => {
    const result = await axios.put(`/api/offers/${id}`, {
      closed: 'declined'
    })
    console.log(result)
  ...
  return(
    <div class="container mt-5">
      ...
      <h3 class="mb-4">Offers Awaiting Your Response</h3>
      <div class="row">
      {offers?.filter(a => a.closed === null).map((x) => (
              ...
              <div class="card-body">
                <div class="row">
                  <div class="col-5">
                    ...
                    <img class="card-img" src={x.sellerlistings_img[0]} alt="product" />
                    <div class="card-body text-center pl-0 pr-0">{x.sellerlistings_title}</div>
                  </div>
                ...
                <div class="row">
                  <button
                    ...
                    onClick={() => acceptOffer(x)}
                  >
                    Accept
                  </button>
                  <button
                    class="btn btn-danger mt-3 col-12 mx-auto"
                    onClick={() => declineOffer(x)}
                  >
                    Decline
                  </button>
                </div>
